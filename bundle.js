...
var key = "a2a73e7b926c924fad7001ca3111acd55af2ff";
...
var Thesaurus = function () {
  function Thesaurus(submitButton, wordField, canvas) {
    ...
    APIUtil.fetchRandomWords(5).then(function (res) {
      _this.board.addWords(res.data.map(function (datum) {
        return datum.word;
      }));
    });
  }
  ...
  _createClass(Thesaurus, [{
    ...
    value: function fetchSynonyms(wordObj, callback) {
      ...
      if (this.store[wordObj.text]) {
        ...
        APIUtil.fetchRelatedWords(wordObj.text).then(function (res) {
          console.log(res);
          var synonyms = res.data.find(function (relation) {
            return relation.relationshipType === "synonym";
          });
          synonyms = synonyms || { words: [] };
          _this2.store[wordObj.text] = synonyms.words;
